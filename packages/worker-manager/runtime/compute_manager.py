...
class ProcessManager:
    ...
    def get_engine(self, engine_key: str) -> ComputeEngine | None:
        qualified_key = self._key(engine_key)
        with self._engines_lock:
            info = self._engines.get(qualified_key)
            return info.engine if info else None

    def get_engine_info(self, engine_key: str) -> EngineInfo | None:
        qualified_key = self._key(engine_key)
        with self._engines_lock:
    ...
    def shutdown_engine(self, engine_key: str, *, emit_snapshot: bool = True) -> None:
        qualified_key = self._key(engine_key)
        namespace, _ = self._split_key(qualified_key)
