...
def test_send_smtp_message_uses_implicit_ssl_for_port_465() -> None:
    msg = _message()
    with patch('backend_core.smtp.smtplib.SMTP') as mock_smtp, patch('backend_core.smtp.smtplib.SMTP_SSL') as mock_smtp_ssl:
        server = MagicMock()
...
def test_send_smtp_message_uses_starttls_when_advertised() -> None:
    msg = _message()
    with patch('backend_core.smtp.smtplib.SMTP') as mock_smtp:
        server = MagicMock()
