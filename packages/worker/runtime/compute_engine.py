...
class PolarsComputeEngine:
    ...
    def _classify_engine_error(exc: Exception) -> tuple[str, dict[str, object]]:
        ...
        if isinstance(exc, OSError) and getattr(exc, "errno", None) == 2:
            return "datasource_metadata_missing", {}
        message = str(exc)
        if ("NO_SUCH_KEY" in message or "RESOURCE_NOT_FOUND" in message) and "GetObject" in message:
            return "datasource_metadata_missing", {}
    ...
    def check_health(self) -> bool:
        ...
        with self._lifecycle_lock:
            ...
            if self.process is None:
                logger.debug(f"Compute process for analysis {self.analysis_id} is absent while marked running. Resetting engine state.")
                self._reset_state()
                return False
    ...
    def shutdown(self) -> None:
        ...
        with self._lifecycle_lock:
            ...
            if process is not None and process.is_alive():
                process.join(timeout=5 if acknowledged else 1)
                if process.is_alive():
                    logger.warning(
