...
- [ ] 4.0 Implement Compute Engine Module
  - [ ] 4.1 Create `backend/modules/compute/schemas.py`:
    - [ ] 4.1.1 Define ComputeExecuteRequest (analysis_id, execute_mode, step_id)
    - [ ] 4.1.2 Define ComputeStatus (job_id, status, progress, current_step, error_message, process_id)
    - [ ] 4.1.3 Define ComputeResult (job_id, status, result_path, row_count, column_count)
    - [ ] 4.1.4 Define PreviewRequest (analysis_id, step_id, limit)
    - [ ] 4.1.5 Define PreviewResponse (columns, data, row_count)
  - [ ] 4.2 Create `backend/modules/compute/operations.py`:
    - [ ] 4.2.1 Implement `apply_filter(df, config)`:
      - Parse filter conditions
      - Apply pl.filter() with conditions
      - Return filtered DataFrame
    - [ ] 4.2.2 Implement `apply_select(df, config)`:
    ...
    - [ ] 4.2.4 Implement `apply_group_by(df, config)`:
      - Get group columns and aggregations
      - Build aggregation expressions
      - Apply df.group_by().agg()
      - Return aggregated DataFrame
    - [ ] 4.2.5 Implement `apply_sort(df, config)`:
      - Get sort columns and directions
      - Apply df.sort()
      - Return sorted DataFrame
    - [ ] 4.2.6 Implement `apply_expression(df, config)`:
    ...
    - [ ] 4.2.8 Implement `load_datasource(datasource)`:
      - Route to correct loader based on source_type
      - Return LazyFrame for large files
  - [ ] 4.3 Create `backend/modules/compute/engine.py`:
    - [ ] 4.3.1 Define PolarsComputeEngine class:
      ```python
  ...
  - [ ] 4.4 Create `backend/modules/compute/manager.py`:
    - [ ] 4.4.1 Define ProcessManager class (singleton):
      ...
      ```
    - [ ] 4.4.2 Implement singleton pattern with get_instance()
    - [ ] 4.4.3 Implement get_or_create_engine():
      - Check if engine exists in dict
      - If not, create new engine and start it
      - Return engine
    - [ ] 4.4.4 Implement get_engine():
      - Return engine from dict or None
    - [ ] 4.4.5 Implement shutdown_engine():
      - Get engine from dict
      - Call engine.shutdown()
      - Remove from dict
    - [ ] 4.4.6 Implement shutdown_all():
      - Iterate all engines
      - Call shutdown on each
      - Clear dict
  - [ ] 4.5 Create `backend/modules/compute/service.py`:
    - [ ] 4.5.1 Implement `execute_analysis(db, analysis_id)`:
    ...
    - [ ] 4.5.2 Implement `preview_step(db, analysis_id, step_id, limit)`:
      - Get analysis from database
      - Get or create compute engine
      - Call engine.preview()
      - Return PreviewResponse
    - [ ] 4.5.3 Implement `get_compute_status(analysis_id)`:
      - Get engine from manager
      - Return status based on engine state
    - [ ] 4.5.4 Implement `cancel_job(analysis_id)`:
      - Get engine from manager
      - Shutdown engine
      - Update analysis status to "draft"
  - [ ] 4.6 Create `backend/modules/compute/routes.py`:
    - [ ] 4.6.1 Create APIRouter with tag="compute"
    - [ ] 4.6.2 Implement `POST /analysis/{id}/run`:
      - Call execute_analysis service
      - Return ComputeResult
    - [ ] 4.6.3 Implement `GET /analysis/{id}/status`:
      - Call get_compute_status service
      - Return ComputeStatus
    - [ ] 4.6.4 Implement `POST /compute/preview`:
      - Accept PreviewRequest body
      - Call preview_step service
      - Return PreviewResponse
    - [ ] 4.6.5 Implement `DELETE /compute/{analysis_id}`:
      - Call cancel_job service
      - Return 204 No Content
  - [ ] 4.7 Register router in `backend/api/v1/__init__.py`
  - [ ] 4.8 Add process manager shutdown to app lifespan:
    - [ ] 4.8.1 In main.py lifespan, call ProcessManager.get_instance().shutdown_all() on shutdown
  - [ ] 4.9 Create `backend/modules/compute/__init__.py`:
    - [ ] 4.9.1 Export router
    - [ ] 4.9.2 Export ProcessManager
    - [ ] 4.9.3 Export schemas
  - [ ] 4.10 Write tests in `backend/tests/modules/compute/test_engine.py`:
    - [ ] 4.10.1 Test engine start/shutdown
    - [ ] 4.10.2 Test execute with simple filter pipeline
    - [ ] 4.10.3 Test execute with group_by pipeline
    - [ ] 4.10.4 Test preview returns limited rows
    - [ ] 4.10.5 Test error handling for invalid pipeline
  - [ ] 4.11 Write tests in `backend/tests/modules/compute/test_routes.py`:
    - [ ] 4.11.1 Test POST /analysis/{id}/run executes analysis
    - [ ] 4.11.2 Test GET /analysis/{id}/status returns status
    - [ ] 4.11.3 Test POST /compute/preview returns sample data
    - [ ] 4.11.4 Test DELETE /compute/{id} cancels job

## Completion Criteria

- [ ] All tests pass (`pytest backend/tests/modules/compute -v`)
- [ ] Can execute analysis pipeline in subprocess
- [ ] Subprocess crash doesn't crash main app
- [ ] Can preview step with sample data
- [ ] Can cancel running job
- [ ] Engines properly shutdown on app exit
