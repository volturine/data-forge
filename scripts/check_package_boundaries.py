...
# Paths that must not reappear as mirrored/neutral ownership copies.
# This is about ownership (who owns models/codecs), not about blocking reuse of
# arbitrary deleted feature paths.
...
PROTO_STRUCT_ALLOWLIST = {
    ...
    'proto/dataforge_protocol/engine_runtime.proto:EngineSubmitJobRequest.payload': 'normalized datasource and operation payloads include provider-defined datasource config and pipeline step JSON',
    'proto/dataforge_protocol/engine_runtime.proto:EngineJobEvent.progress': 'engine progress payloads intentionally preserve existing extensible operation progress events',
    'proto/dataforge_protocol/engine_runtime.proto:EngineJobResult.data': 'engine result data contains arbitrary datasource rows and provider-specific metadata',
    'proto/dataforge_protocol/engine_runtime.proto:EngineJobResult.error_details': 'engine error details are intentionally extensible diagnostics',
    ...
    'proto/dataforge_protocol/worker_runtime.proto:WorkerPublishDatasourceCreateRequest.config': 'created datasource config is provider/user-defined JSON published after worker execution',
...
PROTOCOL_BACKED_ENUM_FILES = [
    ...
    Path('packages/worker/datasources/execution.py'),
    Path('packages/worker/datasources/schemas.py'),
