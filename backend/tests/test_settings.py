...
class TestSettingsMigrations:
    ...
    def test_migrations_move_legacy_encrypted_password_into_smtp_password(self):
        ...
        with engine.connect() as conn:
            ...
            conn.execute(text("INSERT INTO app_settings (id, smtp_password_encrypted) VALUES (1, '0a0b0c')"))
            conn.commit()
    ...
    def test_migrations_keep_smtp_password_when_both_password_columns_exist(self, caplog):
        from core.database import _run_settings_migrations
        ...
        with engine.connect() as conn:
            ...
            conn.execute(text("INSERT INTO app_settings (id, smtp_password, smtp_password_encrypted) VALUES (1, 'keep-me', 'drop-me')"))
            conn.commit()
        ...
        assert row[1] == ''
        assert 'keeping smtp_password' in caplog.text
...
class TestSeedSettingsFromEnv:
    ...
    def test_get_settings_reencrypts_plaintext_password(self, monkeypatch) -> None:
        from modules.settings.models import AppSettings
        ...
        engine = self._make_engine()
        with Session(engine) as session:
            row = AppSettings(id=1, smtp_host='mail.example.com', smtp_password='secret')
            session.add(row)
            session.commit()
            ...
            assert data.smtp_password == 'secret'
            assert row.smtp_password.startswith('enc:')
